...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  const playlistPopupContent = document.querySelector(
    ".playlist-popup-content"
  );
  const playlistEditImg = document.querySelector(".playlis-edit-img");
  ...
  recentMenu.addEventListener("click", async (e) => {
    const recentmenuItem = e.target.closest(".recent-menu-item");
    if (recentmenuItem) {
      recentMenu.classList.remove("show");
      const type = recentmenuItem.dataset.sort;
      const myPlaylists = JSON.parse(localStorage.getItem("myPlaylists"));
      if (myPlaylists) {
        if (type === "1") {
          const sort = myPlaylists.sort((a, b) => {
          ...
          await myPlaylist([sort], "playlist");
        } else if (type === "2") {
          const sort = myPlaylists.sort((a, b) => {
            const dateA = a.acreated_at;
          ...
          const sort = myPlaylists.sort((a, b) => {
            const dateA = a.user_username;
            const dateB = b.user_username;
            if (dateB < dateA) {
              return -1;
            }
            if (dateB > dateA) {
  ...
  //contextmenu
  libraryContent.onmousedown = function () {
    contextMenu.classList.remove("show");
    const libraryItems = document.querySelectorAll(".library-item.active");
  ...
  contextMenu.addEventListener("click", async (e) => {
    ...
    if (unfollowMenuBtn) {
      try {
        const res = await httpRequest.delete(`artists/${data.id}/follow`);
        const artists = await getArtistFollows();
        await myArtistFollows(artists);
    ...
    if (removeMenuBtn) {
      try {
        const res = await httpRequest.delete(`playlists/${data.id}/follow`);
        const playlist = await getMyPlayLists();
        await myPlaylist([playlist], "playlist");
        console.log(res);
        iziToast.info({
  ...
  libraryContent.addEventListener("click", async function (e) {
    e.preventDefault;
    const heroImage = document.querySelector(".hero-image");
    const heroArtistImg = document.querySelector(".hero-artist-img");
    ...
    if (libraryItem) {
      ...
      try {
        ...
        if (tracks.length > 0) {
          ...
          artistHeroSubtitle.textContent = playlist.description || playlist.bio;
          heroArtistImg.src =
            "https://i.pinimg.com/736x/cd/f5/1a/cdf51aaa3c71d3482217d44fc0a09c57.jpg";
          heroImage.src =
            "https://i.pinimg.com/736x/cd/f5/1a/cdf51aaa3c71d3482217d44fc0a09c57.jpg";
          playlist.is_following
            ? (verifiedBadge.innerHTML = `<i class="fas fa-check-circle"></i>
  ...
  playlistSection.addEventListener("click", async (e) => {
    ...
    if (addTrackToList) {
      try {
        toggleMainContent(false, false, false, false, true, true, true, true);
        ...
        if (
          ...
          localStorage.setItem("futurePlayer", JSON.stringify(dataFuture));
        } else {
          localStorage.setItem("futurePlayer", JSON.stringify(dataFuture));
  ...
  uploadImg.addEventListener("change", () => {
    ...
    playlistEditImg.src = URL.createObjectURL(file);
  });
  playlistPopupContent.addEventListener("submit", async (e) => {
    e.preventDefault();
    const playlistPopupName = document.querySelector(".playlist-popup-name");
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const heroArtistImage = document.querySelector(".hero-artist-image");
  ...
  document.addEventListener("click", async function (e) {
    ...
    if (
      playlistPopup.classList.contains("show") &&
      !playlistPopupContent.contains(e.target) &&
      !heroArtistImage.contains(e.target)
    ) {
      playlistPopup.classList.remove("show");
    }
  });
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  contentWrapper.addEventListener("click", async (e) => {
    ...
    const playlistSection = document.querySelector(".playlist-section");
    const heroArtistImage = e.target.closest(".hero-artist-image");
    const playlistPopup = document.querySelector(".playlist-popup");
    ...
    if (followBtnLarge) {
      ...
      if (itemData.type === "artist") {
        try {
          const res = await httpRequest.post(`artists/${itemData.id}/follow`);
